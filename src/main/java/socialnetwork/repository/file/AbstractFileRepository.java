...
public abstract class AbstractFileRepository<ID, E extends Entity<ID>> extends InMemoryRepository<ID,E> {
    ...
    protected AbstractFileRepository(String fileName, Validator<E> validator,boolean skipLoadData){
        super(validator);
        this.fileName=fileName;
    }

    /**
     * loads data from file with name this.fileName
     */
...
//        }
    ...
    private void reloadFile(){
        try (BufferedWriter bW = new BufferedWriter(new FileWriter(fileName))) {
            for (ID id : super.entities.keySet()){
                bW.write(createEntityAsString(entities.get(id)));
                bW.newLine();
            }
        }
        catch (IOException e) {
